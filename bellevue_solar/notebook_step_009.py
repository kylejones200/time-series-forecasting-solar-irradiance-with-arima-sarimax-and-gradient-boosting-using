...
def notebook_step_009() -> None:
    ...
    model.cost = pyo.Objective(expr=sum((model.vol[c] * cost[c] for c in model.crudes)), sense=pyo.minimize)

    model.total_volume = pyo.Constraint(expr=sum((model.vol[c] for c in model.crudes)) == target_volume)

    model.sulfur_limit = pyo.Constraint(expr=sum((model.vol[c] * sulfur[c] for c in model.crudes)) <= sulfur_max * target_volume)

    model.api_limit = pyo.Constraint(expr=sum((model.vol[c] * api[c] for c in model.crudes)) >= api_min * target_volume)
