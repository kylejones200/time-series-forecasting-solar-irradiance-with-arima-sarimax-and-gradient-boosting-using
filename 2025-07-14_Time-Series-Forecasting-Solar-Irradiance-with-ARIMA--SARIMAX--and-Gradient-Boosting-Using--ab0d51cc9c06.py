...
def run_forecasting_pipeline(df, exog_vars, target="GHI", horizon=30):
    ...
    mask = y_true != 0
    mape_arima = np.mean(np.abs((y_true[mask] - arima_forecast[mask]) / y_true[mask])) * 100 if np.any(mask) else np.nan
    mape_sarimax = np.mean(np.abs((y_true[mask] - sarimax_forecast[mask]) / y_true[mask])) * 100 if np.any(mask) else np.nan
    mape_gbt = np.mean(np.abs((y_true[mask] - gbt_forecast[mask]) / y_true[mask])) * 100 if np.any(mask) else np.nan
...
# Run forecast
results, forecasts = run_forecasting_pipeline(df, exog_vars=["Temperature", "Humidity", "Wind Speed"])
...
plt.text(forecast_start, plt.ylim()[1]*0.95, "Forecast start", ha="right", va="top", fontsize=9)
