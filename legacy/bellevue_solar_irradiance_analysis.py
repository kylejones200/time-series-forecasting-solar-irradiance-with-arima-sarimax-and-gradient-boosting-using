...
def forecast_lstm(data, exog_vars, target="GHI", horizon=30, lookback=14):
    ...
    for i in range(horizon):
        ...
        next_input = (
            model(torch.tensor([last_block], dtype=torch.float32)).detach().numpy()
        )
        last_block = np.vstack(
            [last_block[1:], np.hstack([next_input[0], scaled[-horizon + i, 1:]])]
        )
    ...
    preds = scaler.inverse_transform(
        np.column_stack([preds_scaled, np.zeros((horizon, len(exog_vars)))])
    )[:, 0]
    return preds
...
def build_a_directed_network_graph() -> None:
    ...
    nx.min_cost_flow(G, demand={...}, capacity="capacity")
